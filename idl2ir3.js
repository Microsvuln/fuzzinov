...
function generateValue(type) {
    switch (type) {
      ...
      case 'BaseAudioContext':
        return 'new AudioContext()';  // Generate a new AudioContext instance
      ...
      if (constructors.length > 0) {
        ...
        constructors.forEach((constructor, index) => {
          ...
          constructor.arguments.forEach((argument, argumentIndex) => {
            fdirSource += `        const arg${argumentIndex} = ${generateValue(argument.idlType.idlType)};\n`;
          });
...
const fdirSource = generateFDIR(parsedWebIDL);
